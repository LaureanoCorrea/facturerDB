...
class UploadController {
	...
	uploadCSV = async (req, res) => {
		try {
			...
			const fechaSubida = moment().format('DD/MM/YYYY HH:mm:ss');
			...
			await this.deleteOldRecords(ciclo, semana, req.user._id);
			...
			// En lugar de redireccionar, se devuelve una respuesta JSON
			res.status(200).json({
	...
	deleteFileIfExists = (filePath) => {
		if (fs.existsSync(filePath)) {
			...
			console.log(`Archivos eliminados luego de ser procesados ${filePath}`);
		} else {
	...
	// Método para agrupar asesores basados en los pedidos
	...
	deleteOldRecords = async (ciclo, semana, userId) => {
		try {
			const result = await DataDao.deleteByCicloAndSemana(
				ciclo,
				semana,
				userId
			);
			console.log(
				`Registros eliminados para ciclo ${ciclo}, semana ${semana}, usuario ${userId}:`,
				result
			);
		} catch (error) {
